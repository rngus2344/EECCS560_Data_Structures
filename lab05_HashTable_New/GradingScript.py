...
def compileStudentCpp(stu_cpp_file,stu_output_name,inputfoler_name):
    ...
    stu_file = "g++ -std=c++11 "+str(stu_cpp_file)+" -o Stu_Lab5 >/dev/null 2>&1"
    out_path = "./"+stu_output_name+"/stu_result"
    flag = os.system(stu_file)
    if flag ==0 :
        for k in range(10):
            cmd_cpp_2 = "./Stu_Lab5 "+folder_path+str(k)+" > "+ out_path+str(k)
            os.system(cmd_cpp_2)
        return True
    else:
...
if __name__ == '__main__':
    ...
    if((foldername)=="" or (headername)=="" or outputname==""):
        ...
        if(os.path.exists(headername)):
            ...
            header_rename = "cp " +(headername)+ " myHashTable.h >/dev/null 2>&1"
