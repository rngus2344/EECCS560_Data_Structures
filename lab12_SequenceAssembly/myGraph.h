...
public:
  ...
  int findOverlaps( std::string str1, std::string str2, std::string & str )
  {
    int max = -1000;
    int leng1 = str1.length( );
    int leng2 = str2.length( );

    for( int i=1; i<=std::min( leng1, leng2 ); i++ )
    {
      if( str1.compare( leng1-i, i, str2, 0, i ) == 0 )
      {
        if( max < i )
        {
          max = i;
    ...
    for( int i=1; i<=std::min( leng1, leng2 ); i++ )
    {
      if( str1.compare( 0, i, str2, leng2-i, i ) == 0 )
      {
        if( max < i )
        {
          max = i;
  ...
  void assembleFragments( std::vector<std::string> & fragments, std::string & assembly )
  {
    ...
    while( leng != 1 )
    {
      ...
      for( int i=0; i<leng; i++ )
      {
        ...
        {
          std::string str;
          int temp = findOverlaps( fragments[i], fragments[j], str );
      ...
      if( max == -1000 )
        fragments[0] += fragments[leng];
      else
      {
        fragments[l] = tempStr;
        fragments[r] = fragments[leng];
      }
    }
