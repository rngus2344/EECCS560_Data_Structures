...
def compileStudentCpp(stu_cpp_file,stu_output_name,inputfoler_name):
    ...
    stu_file = "g++ -std=c++11 "+str(stu_cpp_file)+" -o StuLab8"
    out_path = "./"+stu_output_name+"/stu_result"
    flag = os.system(stu_file)
    if flag ==0 :
        for k in range(10):
            cmd_cpp_2 = "./StuLab8 "+folder_path+str(k)+" > "+ out_path+str(k)
            os.system(cmd_cpp_2)
...
if __name__ == '__main__':
    ...
    if((foldername)=="" or (headername)=="" or outputname==""):
        ...
        if(os.path.exists(headername)):
            ...
            if "mySkewHeap" in headername and "myLeftistHeap" in headername1:
                header_rename = "cp " +(headername)+ " mySkewHeap.h >/dev/null 2>&1"
                header_rename1 = "cp " +(headername1)+ " myLeftistHeap.h >/dev/null 2>&1" 
            else:
                header_rename = "cp " +(headername)+ " myLeftistHeap.h >/dev/null 2>&1"
                header_rename1 = "cp " +(headername1)+ " mySkewHeap.h >/dev/null 2>&1"          
            os.system(header_rename)
            os.system(header_rename1)
