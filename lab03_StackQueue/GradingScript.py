...
def compileStudentCpp(stu_cpp_file,stu_output_name,inputfoler_name):
    ...
    stu_file = "g++ -std=c++11 "+str(stu_cpp_file)+" -o StuLab3"
    out_path = "./"+stu_output_name+"/stu_result"
    flag = os.system(stu_file)
    if flag ==0 :
        for k in range(10):
            cmd_cpp_2 = "./StuLab3 "+folder_path+str(k)+" > "+ out_path+str(k)
            os.system(cmd_cpp_2)
...
if __name__ == '__main__':
    ...
    if((foldername)=="" or (headername)=="" or outputname==""):
        ...
        if(os.path.exists(headername)):
            ...
            if "myQueue" in headername and "myStack" in headername1:
                header_rename = "cp " +(headername)+ " myQueue.h >/dev/null 2>&1"
                header_rename1 = "cp " +(headername1)+ " myStack.h >/dev/null 2>&1" 
            else:
                header_rename = "cp " +(headername)+ " myStack.h >/dev/null 2>&1"
                header_rename1 = "cp " +(headername1)+ " myQueue.h >/dev/null 2>&1"          
